...
from .protocols import ExchangeProtocol as Exchange
...
class EntityExtractor:
    ...
    """
    def __init__(self, coins: List[str], config: Dict[str, Any], logger: logging.Logger):
    ...
    def find_closest_symbol(self, input_symbol: str) -> Optional[str]:
        ...
        if input_symbol in self.custom_mapping:
            self.logger.info(f"Custom mapping found: {input_symbol} -> {self.custom_mapping[input_symbol]}")
    ...
    def _extract_intent(self, text: str, is_english: bool) -> Optional[str]:
        ...
        if is_english:
            ...
            if match:
                ...
                self.logger.info(f"Intent matched (English): '{intent}'")
                return intent
        else:
            # 한글: intent_map 기반 키워드 매칭
            for keyword, intent in self.intent_map.items():
                if keyword in text:
                    self.logger.info(f"Intent matched (Korean): '{keyword}' -> '{intent}'")
                    return intent
        return None
    ...
    def extract_entities(self, text: str) -> Dict[str, Any]:
        ...
        self.logger.info(f"{lang_type} command detected.")
...
class TradeExecutor:
    ...
    def __init__(self, exchange: Exchange, quote_currency: str, logger: logging.Logger):
